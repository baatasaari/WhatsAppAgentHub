...
<!-- End AgentFlow Widget -->`;
  ...
  app.get("/api/litellm/models", authenticate, requireApproved, async (req: AuthenticatedRequest, res) => {
    try {
      const models = await storage.getLiteLLMModels();
      res.json(models);
    } catch (error) {
      await logger.logError(error as Error, 'litellm_get_models', req.user?.id, undefined, {});
      res.status(500).json({ message: "Failed to fetch LiteLLM models" });
    }
  });

  app.post("/api/litellm/sync-models", authenticate, requireSystemAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      const availableModels = await litellm.getAvailableModels();
      await storage.syncLiteLLMModels(availableModels);
      
      await logger.logSystem('litellm_models_synced', req.user?.id, undefined, true, {
        modelCount: availableModels.length,
      });

      res.json({ message: "Models synced successfully", count: availableModels.length });
    } catch (error) {
      await logger.logError(error as Error, 'litellm_sync_models', req.user?.id, undefined, {});
      res.status(500).json({ message: "Failed to sync LiteLLM models" });
    }
  });

  app.get("/api/litellm/usage-stats", authenticate, requireApproved, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const usage = await storage.getLiteLLMUsageByUser(userId, startDate, endDate);
      const analytics = await storage.getLiteLLMAnalyticsByUser(userId);
      ...
      const successRate = analytics.length > 0
        ? analytics.reduce((sum, a) => sum + (a.successfulRequests || 0), 0) / 
          analytics.reduce((sum, a) => sum + (a.totalRequests || 0), 1) * 100
        : 100;

      // Top models by usage
      const modelUsage = usage.reduce((acc: any, u) => {
        if (!acc[u.modelId]) {
          acc[u.modelId] = {
            id: u.modelId,
            name: u.modelId,
        ...
        acc[u.modelId].totalRequests++;
        acc[u.modelId].totalCost += u.totalCost || 0;
        acc[u.modelId].avgResponseTime += u.responseTime || 0;
        return acc;
      }, {});
      ...
      const topModels = Object.values(modelUsage)
        .map((model: any) => ({
          ...model,
          avgResponseTime: model.avgResponseTime / model.totalRequests,
        }))
      ...
      await logger.logError(error as Error, 'litellm_usage_stats', req.user?.id, undefined, {});
      res.status(500).json({ message: "Failed to fetch usage statistics" });
    }
  });

  app.get("/api/litellm/recommendations", authenticate, requireApproved, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
      const recommendations = await enhancedLLM.getModelRecommendations(userId);
      res.json(recommendations);
    } catch (error) {
      await logger.logError(error as Error, 'litellm_recommendations', req.user?.id, undefined, {});
      res.status(500).json({ message: "Failed to get model recommendations" });
    }
  });

  app.post("/api/litellm/test-model/:modelId", authenticate, requireApproved, async (req: AuthenticatedRequest, res) => {
    try {
      const { modelId } = req.params;
      const userId = req.user!.id;
      
      const testResult = await enhancedLLM.testModelPerformance(modelId, userId);
      res.json(testResult);
    } catch (error) {
      await logger.logError(error as Error, 'litellm_test_model', req.user?.id, undefined, { modelId: req.params.modelId });
      res.status(500).json({ message: "Failed to test model performance" });
    }
  });

  app.post("/api/litellm/chat", authenticate, requireApproved, async (req: AuthenticatedRequest, res) => {
    try {
      const userId = req.user!.id;
      const { model, messages, temperature, maxTokens, agentId, conversationId } = req.body;

      if (!model || !messages) {
        return res.status(400).json({ message: "Model and messages are required" });
      }

      const response = await enhancedLLM.generateResponse({
        model,
        messages,
        temperature,
        maxTokens,
      ...
      await logger.logError(error as Error, 'litellm_chat', req.user?.id, req.body.agentId, {
